...
currentAccount: any;
    ...
    loadAll() {
        this.serviceSettingsService.query({
            page: this.page - 1,
            size: this.itemsPerPage,
            sort: this.sort()}).subscribe(
                (res: HttpResponse<ServiceSettings[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/service-settings'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/service-settings', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: ServiceSettings) {
        return item.id;
    }
    registerChangeInServiceSettings() {
        this.eventSubscriber = this.eventManager.subscribe('serviceSettingsListModification', (response) => this.loadAll());
    }
