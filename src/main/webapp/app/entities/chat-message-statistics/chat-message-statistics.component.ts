...
@Component({
    selector: 'jhi-chat-message-statistics',
    templateUrl: './chat-message-statistics.component.html'
})
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/chat-message-statistics'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/chat-message-statistics', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
